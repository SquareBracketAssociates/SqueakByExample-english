...
\fi
%=================================================================
\chapter{Collections}\label{cha:collections}
...
%=========================================================
\section{The Varieties of Collection}
\label{sec:varieties}
...
Each collection may implement or not such protocols and when they do interpret them to fit with their semantics. We suggest you to browse the classes themselves to identify specific and more advanced protocols.
...
There are several ways to create instances of collections. The most generic ones use the methods \mthind{Collection class}{new:} and \mthind{Collection class}{with:}. \ct{new: anInteger} creates a collection of size \ct{anInteger} whose elements will all be \ct{nil}.  \mthind{Collection class}{with:} \ct{anObject} creates a collection and adds \ct{anObject} to the created collection. Different collections will relize this behavior differently.
...
Use only \ct{Symbol}s as keys for a \ct{IdentityDictionary}, and \ct{String}s (or other objects) as keys for a plain \ct{Dictionary}.


Note that the global \glbind{Smalltalk} is an instance of \clsind{SystemDictionary}, a subclass of \ct{IdentityDictionary}, hence all its keys are \ct{Symbol}s (actually, \ct{ByteSymbol}s, which only contain 8-bit characters).
...
The set operations \emph{union}, \emph{intersection} and \emph{membership} testing are implemented by the \ct{Collection} messages \mthind{Collection}{union:}, \mthind{Collection}{intersection:} and \mthind{Collection}{includes:}.
...
As we explain below set elements are accessed using iterators (see \secref{iterators}).

%---------------------------------------------------------
\subsection{SortedCollection}
In contrast to an \ct{OrderedCollection}, a \clsindmain{SortedCollection} maintains its elements in sort order. By default, a sorted collection uses the message \mthind{Magnitude}{<=} to establish sort order, \ie it can sort instances of subclasses of the abstract class \clsind{Magnitude}, which defines the protocol of comparable objects (\mthind{Magnitude}{<}, \mthind{Magnitude}{=}, \mthind{Magnitude}{>}, \mthind{Magnitude}{>=}, \mthind{Magnitude}{between:and:}...).
(See \charef{basic}.)
...
Imagine you want to sort objects that do not define the method \ct{<=} or that you would like to have a different sorting criterion. You can do that by specifying a two argument block. For example, the class \ct{Color} is not a Magnitude and it does not implement the method \ct{<=}, but we can specify a block stating that the colors should be sorted according to their luminance (a measure of brightness).
...
Instead, with \mthind{String}{copyReplaceAll:} a new string is created.
(Curiously, here the arguments are substrings rather than individual characters, and the sizes do not have to match.)

\begin{code}{@TEST |s| s:= 'No worries' copy.}
s copyReplaceAll: 'rries' with: 'mbats' --> 'No wombats'
\end{code}

A quick look at the implementation of these methods reveals that they are defined not only for \ct{String}s, but for any kind of \ct{SequenceableCollection}, hence the following also works:
...
It is possible to ask whether a string matches a pattern by sending the \mthind{String}{match:} message.
...
\paragraph{Some tests on strings.} The  following examples illustrate the use of \mthind{String}{isEmpty}, \mthind{String}{includes:} and \mthind{String}{anySatisfy:} which are further messages defined not only on Strings but more generally on collections. 
...
In \st loops and conditionals are simply messages sent to collections or other objects such as integers or block (see also \charef{syntax}). In addition to low-level messages such as \ct{to:do:} which evaluates a block with an argument ranging from an initial to a final number, the \st collection hierarchy offers various high-level iterators. Using such iterators will make your code more robust and compact. 
...
For the indexed collections (\ct{Array}, \ct{OrderedCollection}, \ct{SortedCollection}) the method \mthind{SequenceableCollection}{doWithIndex:} gives also access to the current index. This method is related to \ct{to:do:} which is defined on \ct{Number}.
...
When the method \mthind{Dictionary}{do:} is sent to a dictionary, the elements taken into account are the values, not the associations. The proper methods to use are \mthind{Dictionary}{keysDo:}, \mthind{Dictionary}{valuesDo:}, and \mthind{Dictionary}{associationsDo:}, which iterate respectively on keys, values or associations.
...
\paragraph{\mthind{Collection}{count:}} The message \ct{count:} returns the number of elements matching a block.
...
\paragraph{\mthind{OrderedCollection}{anySatisfy:}} The message \ct{anySatisfy:} returns whether at least one element satisfies a condition.
...
Indeed the method \ct{add:} returns the element added and not the receiver.
...
A difficult error to spot is when you redefine \ct{=} but not \ct{hash}. The symptoms are that you will lose elements that you put in sets or other strange behavior. One solution proposed by Kent Beck is to using \ct{xor:} to redefine \ct{hash}.
...
Another nasty problem that can arise is to use mutable objects, \ie that may change their hash value over time, as elements of a \ct{Set} or as keys of a \ct{Dictionary}.
...
\begin{itemize}
  \item A key distinction is between \ct{SequenceableCollection}s, which maintain their elements in a given order, \ct{Dictionary} its subclasses, which maintain key-to-value associations, and \ct{Set}s and \ct{Bag}s, which are unordered.
