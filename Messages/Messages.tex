...
Although \st's message syntax is extremely simple, it is unconventional and can take some getting used to.
...
1 to: 10                        --> (1 to: 10)  "create an interval"
Color r: 1 g: 0 b: 0       --> Color red  "create a new color"
...
\important{Keyword based  messages are messages that require one or more arguments. Their selector consists of one or more keywords each ending in a colon (\ct{:}). They follow the syntactic template: \\
...
The message composition rules are simple but they result in inconsistency for the evaluation of arithmetic expressions expresed in terms of binary messages. Here we see the common situations where extra parentheses are needed. 
...
The characters \ct{[}, \ct{]}, and \ct{(}, \ct{)} 
...
When the characters \ct{[}, \ct{]}, and \ct{(}, \ct{)} surround some words with colons, these words participate in the keyword message \emph{local} to the area defined.
...
\important{
The characters \ct{[}, \ct{]}, and \ct{(}, \ct{)} delimit distinct areas. Within such an area, 
a keyword message is the longest sequence  of words terminated by \ct{:} that is not cut by the characters \ct{.},  or \ct{;}. 
When the characters \ct{[}, \ct{]}, and \ct{(}, \ct{)} surround some words with colons, these words participate in the keyword message local to the area defined.}
...
The following expression does not require parentheses because the expression  \ct{x isNil} is unary hence is sent prior to the keyword message \lct{ifTrue:}.
\begin{code}{}
(x isNil)
   ifTrue:[...]
\end{code}
...
No object is created, so in \lct{(\emph{expression})}, the \lct{\emph{expression}} will \emph{always} be evaluated exactly once (assuming the sounding code is evaluated once).
...
Expressions (\ie messages sends, assignments ...) separated by periods are evaluated in sequence.
...
\item Binary messages are messages that involve two objects, the receiver and another object \emph{and} whose selector is composed of  one or two characters from the following list: \ct{+}, \ct{-}, \ct{*}, \ct{/}, \ct{|}, \texttt{\&}, \ct{=}, \ct{>}, \ct{<}, \texttt{\~}, and \ct{@}.
