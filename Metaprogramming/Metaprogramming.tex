...
%-------------------------------------------------------------------------
\subsection{Methods as Objects} %\seclabel{compiledmethods}
...
These methods are stored in a dictionary (a \emph{method dictionary}), keyed by their names. We can access the method dictionary of a class by sending the message \ct{methodDict} to this class. For instance, executing \ct{ExampleClass methodDict} return the methods dictionary of the class \ct{ExampleClass}.
...
%-------------------------------------------------------------------------
\subsection{Objects as Methods}% \seclabel{reificationexecution}
...
For instance, a method \ct{foo:} can be added to the class \ct{C} by evaluating:
...
As similarly shown in \secref{compiledmethods}, the last line above adds an entry named \ct{foo:} to the method dictionary of \ct{C}. Instance of \ct{C} therefore understand messages named \ct{foo:}. Because of the method execution reification mechanism, evaluating \ct{c foo: 10} is in fact evaluated by the VM as:
