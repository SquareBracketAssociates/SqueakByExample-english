...
\ab{I'm reviewing this chapter.  I changed the title back to the other one, since it is more descriptive.}
...
Modifying the code in a package with the system browser makes that package \emph{dirty}.
This means that the version package in the image is different from the version that has been saved or loaded. 
In Monticello, the name of a dirty package is preceded by an asterisk (\ct{*}) . This indicates to the programmer which package have uncommitted changes, and therefore need to be saved into a repository if those changes are not to be lost. Saving a dirty package cleans it.
