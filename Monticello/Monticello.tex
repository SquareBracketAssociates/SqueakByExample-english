...
\begin{figure}[ht]\centering
	...
	\caption{Repositories under SqueakSource are configurable.\figlabel{squeaksourcesetting}}
\end{figure}

The configuration of a Monticello repository offers a large range of options (cf. \figref{squeaksourcesetting}): tags may be assigned, a license may be chosen, access for people that are not part of the project may be restricted (read/write, read, no access), emails may be sent upon commits, mailing list, and users may be part of the project (as administrator, developer, or guest).
...
\ab{This is out of place.   First, this definition already appears in the ``Environment'' chapter.  Second, it needs to come before the section on the snapshot browser, which shows us an extension.} \alex{Why is it out of place? The mechanism belongs to Monticello}
...
\noindent
Note that right-clicking the right-hand side of the inspector opens a menu with different sorting options. The \menu{unchanged} entry in the menu discards any particular sorting. It uses the order given by the repository.
...
When Monticello loads a package into the image, any class that defines an \ct{initialize} method on its class side will be the receiver of a message send toward this method. The message \ct{initialize} is sent to classes that defines this class method only. A class that does not define this method will not be initialized, even if defined on one of its superclass.
...
%:\alex{Maybe this is to avoid double initialization}
...
As an illustration, let us suppose an extension of the Quinto game to support unchangeable cells. Those cells are located randomly on the board. Generate numbers need to be generated. This can be done by adding a variable \ct{randomNumber} to the class \ct{SBEGame}. This kind of extension is not well supported by Monticello. A package cannot define the variable \ct{randomNumber} as an extension of \ct{SBEGame}. Monticello supports method extension only.
...
%:\alex{Probably, but this seems to be broken with Monticello}
...
%:\ab{eh?  Do you mean that \emph{a particular user} may have restricted access rights?}

\paragraph{HTTP} HTTP repositories are probably the kind of repository the most used since it is favored by SqueakSource. % (Such servers can also be configured for read-only access. Saving versions via HTTP uses the PUT method \ab{Is ``put'' the right word}, which must be enabled on the server.)
...
%:\ab{And which of the nice properties listed above apply?}
...
%:\ab{I would like a couple of sentences about GOODS, which I've never heard of.}
...
%:\ab{Is this an example of a write-only repository?} 
