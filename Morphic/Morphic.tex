...
\dothis{If you don't have a \menu{Supplies} flap at the bottom of your screen, execute \ct{Flaps enableGlobalFlaps} to create one.  Drag a yellow ellipse and a white star from the \menu{Supplies} flap.  Place the star over the ellipse, and control-click the star to bring up a menu\footnote{You can also blue-click the star to bring up the morphic halo, and then click the red menu handle.}.   Select \menu{embed into \go Ellipse}.  Now the star and the ellipse move together.}
...
\dothis{Blue click twice on the white star.  Drag the star away from the ellipse using the grab handle \grabHandle{}.}
...
The morphic framework sends the message \ct{drawOn:} to a morph when it needs to redisplay the morph on the screen.  The parameter to \ct{drawOn:}  is a kind of \clsind{Canvas}; the expected behavior is that the morph will draw itself on that canvas, inside its bounds.
...
\dothis{To change this behavior, change the color of the rectangle morph to \ct{Color blue} using an inspector.  Blue rectangles should be accepted by the \ct{ReceiverMorph}.}
