...
When you start using \sq, it can be surprisingly helpful to actually label your mouse, as shown in \figref{colouredMouse}.
...
\dothis{Red-click again on the \sq background.}
...
Put the blob down (by red-clicking) somewhere. It will start to move.}
\index{Morphic}
...
This reflects the idea that objects are responsible for their own actions, so you should never tell an object what to do\,---\,instead you ask it politely by sending it a message. The object selects the method for responding to your message, not you!


%=================================================================
\section{Keyboard shortcuts}

If you want  to evaluate an expression, you do not always have to bring up the yellow-button menu. Instead, you can directly use \ind{keyboard shortcuts}. These are the parenthesized expressions in the menu.  Depending on your platform, you may have to press one of the modifier keys (control, ALT, command, or meta).
...
The left panel allows us to browse the instance variables of an object, which are shown in the right panel.
...
As we shall see, there are several interesting class browsers available for \sq, but this is the basic one you will find in any image.
...
The title indicates that we are browsing the class \clsind{Object}.
...
The middle pane is the \emph{class definition}.
...
The bottom pane shows the \emph{class comment} --- a piece of plain text describing the class.
...
The idea behind Test Driven Development, also called Behavior Driven Development or just TDD, is that we write a test that defines the desired behavior of our code \emph{before} we write the code itself.
...
\begin{itemize}
  ...
  \item Use the \ind{red button} on the \sq background to bring up the \emph{World menu} and launch various tools. You can also launch tools from the \toolsflapind at the right of the \sq screen.
