...
The \ct{increase} and \ct{decrease} methods don't change \ct{0} into \ct{1} or \ct{3} into \ct{2}.
\ab{Can someone tell me WHY the macros add that extra space after a code word?  Why should I have to put an $\backslash$mbox around every one justin case it comes at the end of a line?}
...
When the state of a \mbox{\ct{WACounter}} object is replaced by a previous state, the \emph{value} of the whole object is replaced by a previous value, which correctly replaces the current value of \ct{count} by a prior value. 
\index{Seaside!backtracking state}
...
The current value of the counter is displayed as an XHTML heading, and the increment and decrement operations are implemented as html anchors (that is, links) with callbacks to blocks that will send the counter object \ct{increase} and \ct{decrease}.
% (not shown).
...
\noindent
\ct{WAMultiCounter} is also a standalone application, so it overrides \mthind{WAComponent class}{canBeRoot} to answer \ct{true}.
...
The message \ct{render: anyObject} can be sent to a canvas to render \ct{anyObject}, though it is normally used to render subcomponents.  \ct{anyObject} will itself be sent the message \ct{renderContentOn:}
This is what happens in our multi-counter (see \figref{WAMultiCounter}).
...
\ab{I don't understand this point.  Why?}
...
The method \ct{SeasideDemoWidget>>>style} defines the display attributes for XHTML elements with these attributes (see \secref{css}). \ab{eh?}
...
\begin{itemize}
  ...
  It is a subclass of \ct{WAComponent} and the direct superclass of the following three classes. \alex{four classes?}
