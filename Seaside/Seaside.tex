...
The second, with the magnifying glass, opens an object inspector on the currently active instance. 
...
Set the root component to \clsind{WACounter}, then \link{Save} and \link{Close}.
...
\caption{The \ct{WACounter} component\ab{It took me a while to realize that this figure shows both class and instance methods, and that it uses underlining to indicate class methods.  Is this a convention that we have used elsewhere?  Is it UML? I found it confusing for SMalltalk, where the class and instance methods are on different objects.  How about using two columns of method names, with the instance methods on the left and the class methods on the right?}}
\label{fig:WACounter}
\end{center}
\end{figure}
...
It must also automatically register itself as an application; this is done in its class-side \ct{initialize} method (see \figref{WACounter}).
...
In this case \ct{WACounter} asks to track its own state by returning \ct{Array with: self}. \ab{This paragraph has lost me.  I think that I get the general idea, but not the details.  Another new term\,---\,``tracked''\,---\,is introduced without being explained.}

\paragraph{\emph{Caveat.}}
There is a subtle but important point to watch for when declaring state for backtracking. \ab{Does this refer back to the previous paragraph?  It uses different words.}
...
In addition, it is a composite component, so Seaside requires it to declare its children by implementing a method \ct{children}, which returns an array of all the components it contains.
