...
\ct{callback:} specifies a block that is triggered \on{on the client-side} when the user presses the button.
...
\on{Isn't the point here that the rendering within the javascript callback is *client-side*, so it is a different renderer than the html renderer we have in the enclosing scope?}
...
Try the revised application.  Note that the feedback is much more natural.
In particular the URL is not refreshed every time you enter a digit.
