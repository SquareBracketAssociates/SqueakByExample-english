...
\ct{WASnapShot} is a subclass of \ct{IdentityDictionary} that registers each object to be tracked as a key, and a (shallow) copy of its state as a value.
If the state is restored from a snapshot, all registered objects are restored to the saved value.
In the case of the counter, the state to be backed up is presumably a \ct{SmallInteger}.
...
(Have a look at \ct{WAFileLibrary}, which provides a way to serve static files without the need for a standalone server.)
...
The message \ct{#request:} performs a call to a component that will let you edit a text field.
...
The message \ct{#inform:} calls a component that simply displays the argument message and waits for the user to click ``ok''.
...
The message \ct{#confirm:} asks a questions and waits for the user to select either ``Yes'' or ``No''.
...
A few further convenience methods, such as \ct{chooseFrom:caption:}, are defined in the \prot{convenience} protocol of \ct{WAComponent}.
...
The first is a \ct{WAChoiceDialog} to choose a cheese (generated by the convenience method \lct{\#chooseFrom:caption:}).
...
%:-- Start stack machine implementation
...
%:-- minimum 20 minutes to implement stack machine with tests
...
\dothis{
Use the two helper methods to script the keypad as shown in \figref{renderKeypad}.
}


%:FIG Keypad rendering -- renderKeypad
\begin{figure}[t]
\begin{code}{}
...
\end{code}
\caption{Rendering the keypad}
\label{fig:renderKeypad}
\end{figure}
...
You will need an html anchor that look something like this:
}
