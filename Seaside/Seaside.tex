...
\ab{I'm reviewing this chapter}

%=================================================================

\ab{I don't know why this chapter does not use the method environment for its methods.  Wasn't that the point of defining it?}
...
Seaside includes a web server; you can turn the server on, listening on port 8080, by evaluating
\clsind{WAKom} \ct{startOn: 8080}
and you can turn it off again by evaluating
\ct{WAKom stop}.
In the prepackaged installation, \ab{is this referring to the ``one-click'' image?}the default \subind{Seaside}{administrator login} is \lct{admin} and the default password is \lct{seaside}.
To change them, evaluate: \clsind{WADispatcherEditor} \ct{initialize}. \ab{does it prompt me for a new name and password?  Or must I edit the \ct{initialize} method?}

%:FIG seasideStartup
\begin{figure}[tbh]
\begin{center}
\includegraphics[width=\textwidth]{seasideStartup}
\caption{Starting up Seaside \ab{This picture looks ``soft''.  Perhaps it was captured from an anti-aliased screen?  Screen resolution anti-aliasing looks bad when printed in the book at three-times the resolution!}}
...
\noindent
\dothis{Navigate to the \lct{examples{\go}counter} page.}
\ab{we need a picture of this page}
...
\ab{I don't think that we should use the $\backslash$menu macro for links on a web page.  I've changed what I hope are the correct ones  to $\backslash$link, which presently uses underlining, but we could easily change to use colored text, or whatever.}
\link{New Session} will start a new session on the counter application.
\ab{The reader doesn't know what ``a new session on the counter application'' means.  We haven't heard about ``sessions'' yet.}
...
\ab{This is going a bit too fast for me.  I want to be told first that there is a counter object, and maybe see its protocol.  I don't yet know what a ``component'' is.}
...
Note that the source view contains active links.  
\ab{eh?}
Contrast the source view provided by the Halos with the source view offered by your browser.
\ab{What am I supposed to see?}}
...
\ab{Stylistically, I don't like this: three ``dothis''s with no intervening explanation or commentary.}
...
Use the Seaside class browser to view the implementation of \ct{WACounter}.
You should see three methods on the class side and three on the instance side.
}
...
\ab{applications? components? objects?  Shouldn't we explain this terminology somewhere?}
Select \link{Configure} next to ``examples.''
Add a new entry point called ``counter2'' for an \emph{Application}.  \ab{I added this.  Is it correct?}
On the next screen, set the \emph{root component} to \clsind{WACounter}, then \button{Save} and \button{Close}.
...
\ab{I can't follow this without running Seaside.  Maybe that is OK; certainly it is a good way to learn.  But my goal up until now has been to make the book stand alone, so that one can read it on the train or in bed.   Should we add enough pictures to make  that possible here too?}
...
You can put Seaside into deployment mode using either the configuration page or the \link{Configure} button in the toolbar.
In either case, set the deployment mode to \emph{true}.
Note that this affects new sessions only.  \ab{There's that ``sessions'' word again!}
...
If you remove the configuration \ab{``config''?} application, you can get it back by evaluating
\clsind{WADispatcherEditor} \ct{initialize}.

%=================================================================
\section{Seaside components}

Seaside applications are built out of \emph{\subind{Seaside}{components}}.
...
%\alex{is this "allow to" correct?}
%ab{``allowed to be'' is correct.}
...
\caption{The \ct{WACounter} class, which implements the \emph{counter} application.  Methods with underlined names are on the class-slde; those with plain-text names are on the instance side.}
\label{fig:WACounter}
\end{center}
\end{figure}
...
%\alex{increase and decrease are not shown in Figure \ref{fig:WACounter}}
%\ab{they are now!}
...
However, we also want Seaside to synchronize the counter with the browser page; that is, when the user clicks on the browser's ``back'' button, we want seaside to ``backtrack'' the state of the \ct{WACounter} object.
Seaside includes a general mechanism for backtracking, but each application has to tell Seaside which objects to track.  

An application \ab{component?} does this by implementing the \ct{states} method on the instance side:
\ct{states} should return an array containing all the objects to be tracked.
In this case, the \ct{WACounter} object adds itself to Seaside's table of backtrackable objects by returning \ct{Array with: self}. \ab{I tried to make this more readable. I think that I get the general idea, but not the details.  Another new term\,---\,``tracked''\,---\,is introduced without being explained.}
...
\clsind{WASnapshot} object; \ct{WASnapshot} is a subclass of \clsind{IdentityDictionary} that records the objects to be tracked as keys, and shallow copies of their state as values.
...
At any time, the object named by \ct{count} is an integer, and integers are immutable.
The \ct{increase} and \ct{decrease} methods don't change the state of the object \ct{0} into \ct{1} or the object \ct{3} into \ct{2}.
\ab{Can someone tell me WHY the macros add that extra space after a code word?  Why should I have to put an $\backslash$mbox around every one just in case it comes at the end of a line?}
...
\ab{Did we ever explain what the subtle but important point is?}
...
\caption{WAMultiCounter \ab{In all of these figures the method patterns have been omitted to save space.  Unfortunately, this doesn't work for methods that take parameters, like renderContentOn: html.  One possible fix is to put the parameters in the UML box.  Whatever we do, it has to be done uniformly for all the UML figures.}
\ab{Bottom placement of figures seems to be broken}
}
...
The basic idea is that a Seaside component should override the method \mthind{WAPresenter}{renderContentOn:}, which will be called by the framework whenever the component needs to be rendered.
This method will be passed an argument that is an \seeindex{canvas}{html canvas} \emphind{html canvas} for rendering the component.  By convention, the html canvas parameter is called \ct{html}.
...
The message \ct{render: anyObject} can be sent to an html canvas to render \ct{anyObject}, though it is normally used to render subcomponents.  \ct{anyObject} will itself be sent the message \ct{renderContentOn:}
...
\ab{The intuition behind the name \emph{brush} escapes me entirely.  Can you help?}
...
In \figref{RenderingDemo} we can see the output of many of the basic brushes offered by Seaside.\footnote{The source code is in the package \ct{SBE-SeasideDemo} in the project \url{http://www.squeaksource.com/SqueakByExample}.}
...
\ab{Why not use a brace array, it's more readable}.
\begin{code}{}
...
The subcomponent is rendered as an XHTML \emph{DIV} \ab{why capitals?} with the \ind{CSS} class ``subcomponent''.
...
The first helper method \ab{give it a name} shows you how to generate XHTML paragraphs, plain and emphasized text, and images.
...
\ab{I don't understand this point.  Why?  What objects are valid as the argument to a brush?}
...
The next helper method \ab{give it a name} shows you how to generate lists and tables.
...
The method \ct{SeasideDemoWidget>>>style} defines the display attributes for XHTML elements with these attributes (see \secref{css}). \ab{eh?  Maybe this makes sense if one already knows what the html attributes of the divs and spans should be \ldots  but I need a lot more help.}
...
Finally we see a simple example of a link, created by binding a simple \subind{Seaside}{callback} to an anchor. \ab{what's the difference between an anchor and a link?  Previously, the text said ``anchor'', and I added ``i.e., a link)''}
...
\alex{I would stress that toggleValue has to be a class attribute  \ab{You mean instance variable?}. It cannot be a temporary variable (method or block)}
...
Since a form is a complex entity, it is rendered using a block. \alex{I would love to see a better justification for using blocks. It is not that clear}
...
The first argument is the name of an instance variable for which accessors have been defined \alex{Which accessors ? Shall I define heading and heading:, accessor and mutator of the variable heading? Does these accessors have to follow a naming convention ?}, and the second argument is the object to which this instance variable belongs.
...
Don't forget, if you \link{Toggle Halos}, you can directly browse all the source code of the examples on line using the Seaside class browser.
...
\label{fig:democss}}
\end{figure}
\figref{democss} illustrates a simple style sheet for the rendering demo.
...
\ab{In the given style sheet, does $\sharp$ indicate an id and \ct{.} a class?}
Note that a particular XHTML element may have multiple classes, thus enabling a fine degree of control over how display attributes are combined. \ab{Well, how \emph{are} they combined?}

This style sheet expects at most one element to specify the \emph{author} of the web page.
To match an id, the selector \ab{the selector of what?  A Smalltalk Selector?} should include the name of the id preceded by a \ct{#}.
A class name should be preceded by a dot.

\ab{This needs an example of some html that triggers these styles, and a figure of the rendered result.}
...
\begin{enumerate}
  ...
  \ab{Can't we find a better name than ``callee''?}
  \ab{What does it mean to ``temporarily replace'' a component.  What do I see on the web page\,---\,a brief flicker while the component is replaced by another?}

  \item A \emphsubind{Seaside}{task} is a special kind of component that subclasses \clsind{WATask} (instead of \clsind{WAComponent}). \label{sec:task}
...
\ab{This section has me completely lost.  I don't know what we are trying to achieve, so I fear that we will never understand how to do it.}
...
They are shown in \figref{dialogs}.
We can see these convenience methods being used within \ct{SeasideDialogDemo>>>renderContentOn} \ab{cross-reference to this method ??}
...
\ab{Maybe it would be easier to understand this section if these simple examples proceeded the more general discussion of call: answer:  Either way, in addition to the \sq code, I also need to see the web pages generated by the examples.}

%-----------------------------------------------------------------
\subsubsection{Tasks}

A \subind{Seaside}{task} is a component that subclasses \clsind{WATask}.
It does not render anything itself, but simply calls other components in a control flow defined by implementing the method \mthind{WATask}{go}. \ab{This repeats what was already said on page \pageref{sec:task}}
...
\ab{\ct{confirmCheese} and \ct{informCheese} are missing.}
...
The corresponding pages are \emph{invalidated}, and any attempt to go back to them will cause Seaside to generate a warning and redirect the user to the last \ab{most recent} valid page.
...
\clsind{WAStoreTask} captures this workflow sequence. \ab{The name of this class does not appear in the code that you have just shown.}
...
\begin{itemize}
  \item \ct{MyWidget}\,---\,this should be an abstract class that defines the common CSS style sheet for the application, and other common behavior for the components of the RPN calculator.
  It is a subclass of \ct{WAComponent} and the direct superclass of the following four classes. 
  \ab{MyWidget is a poor name.  CalculatorWidget?}
...
Both frameworks \alex{are?} supported in Seaside through the package ``Scriptaculous''.
