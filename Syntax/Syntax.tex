...
\item[Cascades.] Semicolons can be used to send a \emphind{cascade} of messages to a single receiver.  In \ct{Transcript show: 'hello'; cr} we first send the keyword message \ct{show: 'hello'} to the receiver \ct{Transcript}, and then we send the same receiver the unary message \ct{cr}.
	\seeindex{;}{cascade}
...
Instance variables (and Class variables) are initialized to \ct{nil}.
...
When a series of messages is being sent to the \emph{same} receiver, then this can expressed more succinctly as a \emphind{cascade}.
...
The following block refers the variable \ct{x} of its enclosing environment:
...
We can also send the message \mthind{Number}{to:do:} to an number which then acts as the initial value of a loop counter.
...
More about collections and streams can be found in  \charef{collections} and \charef{streams}.
